...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				board after_board = move->after_state();
				float value = 0;
				
				for (int i=0;i<16;i++)
				{
					if (after_board.at(i)==0)
					{
						after_board.set(i,1);
						value += estimate(after_board) * 0.9;
						after_board.set(i,2);
						value += estimate(after_board) * 0.1;
						after_board.set(i,0);
					}
				}
				value /= 16;
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for (path.pop_back();path.size();path.pop_back())
		{
			...
			target = move.reward() + update(move.before_state(),(move.reward()+target-estimate(move.before_state()))*alpha);
